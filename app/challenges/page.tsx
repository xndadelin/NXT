...
function Challenges() {
  ...
  const paginatedChallenges = displayChallenges.slice(
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(displayChallenges.length / itemsPerPage)
  );
  ...
  const rows = paginatedChallenges.map((challenge, index) => (
    ...
    >
      ...
      <Table.Td>
        <Badge
          color={colors[challenge.difficulty as keyof typeof colors] || "gray"}
        >
          {challenge.difficulty}
        </Badge>
      </Table.Td>
      <Table.Td>{challenge.category}</Table.Td>
      <Table.Td>{challenge.upvotes}/{challenge.downvotes}</Table.Td>
  ...
  const onSort = (column: string, colIndex: number) => {
    ...
    const newDirection =
      sortDirection[colIndex] === 0
        ? 1
        : sortDirection[colIndex] === 1
        ? -1
        : 1;

    const newSortDirection = [0, 0, 0, 0, 0, 0, 0];
    ...
    switch (column) {
      ...
      case "Votes":
        newSortedChallenges.sort((a, b) => {
          return newDirection * (((a.upvotes || 0) - (a.downvotes || 0)) - ((b.upvotes || 0) - (b.downvotes || 0)))
        })
        setSortedChallenges(newSortedChallenges);
       break;
       default:
  ...
  return (
    <Container size={"xl"} style={{ display: "flex", flexDirection: "column" }}>
      <Card>
        <Group justify="space-between" p="md" pb="xs">
              ...
              <Menu.Dropdown>
                ...
                {difficulties.map((difficulty) => (
                  <Menu.Item
                    ...
                    leftSection={
                      <Checkbox
                        checked={filterDifficulty.includes(difficulty)}
          ...
          <Group style={{ width: "100%" }}>
            <Text size="sm" c="dimmed">
              Showing {paginatedChallenges.length} of {displayChallenges.length}{" "}
              challenges
            </Text>
