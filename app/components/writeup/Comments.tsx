import { useComments } from "@/app/utils/queries/writeups/useComments";
...
import { addComment } from "@/app/utils/mutations/writeups/addComment";
...
export default function Comments({ writeupId }: CommentsProps) {
  const { comments, loading, error, refetch } = useComments(writeupId);
  const { user } = useUser();
  ...
  const submitComment = async (text: string, parentId: string | null) => {
    ...
    stateUpdater(true);
    try {
      await addComment(writeupId, user.id, text.trim(), parentId);
  ...
  const organizeComments = (commentsArr: Comment[]) => {
  ...
  const { topLevel, replies } = comments
  ...
  return (
    <Card withBorder p="lg" radius="md">
      ...
      {!hasComments && (
        <Text c="dimmed" mb="lg">
          No messages yet. Be the first to start the comments!
        </Text>
      )}
      ...
      {user ? (
        <Group align="flex-start" gap="sm">
          ...
          <Box style={{ flex: 1 }}>
            <Textarea
              placeholder={
                hasComments ? "Add a comment..." : "Start the comments..."
              }
              value={newMessage}
        ...
        <Text c="dimmed" ta="center">
          Please sign in to join the comments
        </Text>
      )}
    </Card>
  );
